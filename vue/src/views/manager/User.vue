...
export default {
  ...
  methods: {
    ...
    handleEdit(row) {   // Edit data
      this.form = JSON.parse(JSON.stringify(row))  // Assign value to the form object, deep copy data
      this.fromVisible = true   // Open the dialog
    },
      ...
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          this.$request({
            ...
            if (res.code === '200') {  // Indicating successful save
    ...
    del(id) {   // Delete single item
      this.$confirm('Are you sure you want to delete?', 'Confirm deletion', {type: "warning"}).then(response => {
        this.$request.delete('/user/delete/' + id).then(res => {
          if (res.code === '200') {   // Indicating successful operation
    ...
    handleSelectionChange(rows) {   // All currently selected rows
    ...
    handleAvatarSuccess(response, file, fileList) {
      // Replace the avatar property with the link to the uploaded image
